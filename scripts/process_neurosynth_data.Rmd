...
database <- read.csv("/Users/Evan/Desktop/Epilepsy_research/neurosynth-master/data/current_data/database.csv")
neurosynth_paper_database <- read.csv("../data/neurosynth/neurosynth_paper_database.csv")

# Yale Brain Atlas positions (whole brain)
atlas_whole_positions <- read.csv("../data/atlas/atlas_whole_positions.csv")

# Yale Brain Atlas indices (whole brain)
...
# Remove all activations greater than 10mm away from nearest vertex
...
atlas_positions_rounded <- atlas_positions
atlas_positions_rounded$x <- round(atlas_positions_rounded$x, 0)
atlas_positions_rounded$y <- round(atlas_positions_rounded$y, 0)
atlas_positions_rounded$z <- round(atlas_positions_rounded$z, 0)

avg_z_score_by_parcel_list <- list()

# For each parcel
for (i in 1:690){
  parcel_of_int <- unique(atlas_positions_rounded$parcel)[i]
  coord_of_int_df <- atlas_positions_rounded[atlas_positions_rounded$parcel == parcel_of_int, ]
...
neurosynth_all_activation_df <- read.csv("../data/neurosynth/neurosynth_all_activation_database.csv")
neurosynth_terms_functional_review_df <- read.csv("../data/neurosynth/neurosynth_terms_functional_review.csv")

# Average activation data for functional terms that we combined into a new shared functional term 
neurosynth_functional_activation_df <- neurosynth_all_activation_df[,which(names(neurosynth_all_activation_df) %in% neurosynth_terms_functional_review_df$original_functional_term)]
# Initialize clean dataframe
neurosynth_functional_activation_clean_df <- as.data.frame(matrix(NA, 690, length(unique(neurosynth_terms_functional_review_df$new_functional_term))))
names(neurosynth_functional_activation_clean_df) <- unique(neurosynth_terms_functional_review_df$new_functional_term)

for (i in 1:ncol(neurosynth_functional_activation_clean_df)){
  new_functional_term_i <- names(neurosynth_functional_activation_clean_df)[i]
  original_functional_terms_i <- neurosynth_terms_functional_review_df$original_functional_term[neurosynth_terms_functional_review_df$new_functional_term == new_functional_term_i]
  original_functional_terms_i_activation <- neurosynth_functional_activation_df[,which(names(neurosynth_functional_activation_df) %in% original_functional_terms_i)]
  if (length(original_functional_terms_i) == 1){
    neurosynth_functional_activation_clean_df[,i] <- original_functional_terms_i_activation
  } else {
    neurosynth_functional_activation_clean_df[,i] <- rowMeans(original_functional_terms_i_activation)
  }
}
...
# Result is average z scores for 334 functional terms across 690 parcel
write.csv(neurosynth_functional_activation_clean_df, "../data/neurosynth/neurosynth_functional_activation_database.csv", row.names = F)
```
...
for (i in 1:length(pmid_list)){
  Sys.sleep(0.15)
  progress(i, length(first_i:last_i))
...
# reduces from 14,200 unique abstracts to 12,909
